...
# This script runs the Tacotron2 training for the LJSpeech25 dataset.
...
OUTDIR=/E2E-thesis/Trained/Training/LJSpeech25/     # substitute later for output location
...
# Copy the LJSpeech_25 dataset archive to the scratch directory
cp -r $DATADIR/LJSpeech_25.tar.gz $SCRATCHDIR

# Navigate to the scratch directory and extract the dataset
cd $SCRATCHDIR
tar -xzf LJSpeech_25.tar.gz
...
singularity exec --nv $SCRATCHDIR/container.sif python train.py --device=cuda:0 --max_grad_norm=1.0 --data_folder=$SCRATCHDIR/LJSpeech_25 hparams/train.yaml
